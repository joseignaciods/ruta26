...
export default function AdminUserDetail({ userId, onBack }) {
  ...
  return (
    <section className="panel-card admin-panel">
      ...
      <label>Rol
        <select disabled={!isSuperadmin} value={data.role} onChange={async event => {
          try {
            await invoke({ action:'set_role', targetUserId:userId, role:event.target.value || null })
      ...
      <div className="two-cols admin-limit-grid">
        <label>Límite IA<input type="number" min="0" value={ai} placeholder={String(data.settings.ai_monthly_limit_default)} onChange={event => setAi(event.target.value)} /></label>
        <label>Límite búsquedas<input type="number" min="0" value={ta} placeholder={String(data.settings.ta_monthly_limit_default)} onChange={event => setTa(event.target.value)} /></label>
      </div>
      ...
      {!data.memberships.length ? <p>Sin viajes.</p> : data.memberships.map(member => (
        <div className="member-line" key={member.trip_id}><span>{member.trips?.name || member.trip_id}<small>{member.trips?.start_date || 'Sin fecha'}</small></span></div>
      ))}
    </section>
  )
}
